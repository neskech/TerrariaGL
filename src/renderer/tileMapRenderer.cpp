...
void TileMapRenderer::init(){
    ...
    EBO.allocateData<unsigned int>((Camera::getCamViewWidth() + 1) * (Camera::getCamViewHeight() + 2 + 1) * 6, GL_STATIC_DRAW);
    addIndexData();
    ...
    shader = AssetPool::getShader("../assets/shaders/worldShader.glsl");
    TexParams params{
    ...
    tileAtlas = AssetPool::getTexture("../assets/img/SpriteSheet.png", params);
...
void TileMapRenderer::updateWorldData(){
      ...
      int y_cord = (int) floor(cameraPos.y + CAM_HEIGHT / 2.0f) + 1;
...
void TileMapRenderer::addTileData(int index, int x, int y, int texID){
    ...
    const float MIP_GAP = 2.0f;
    int offset = index * 4 * VERTEX_SIZE;
    for (int i = 0; i < 4; i++){
        int row = i < 2;
        int col = i == 1 || i == 3;

        float gapX = col == 0 ? MIP_GAP : -MIP_GAP;
        float gapY = row == 0 ? MIP_GAP : -MIP_GAP;
        buffer_ptr[0 + i * VERTEX_SIZE + offset] = x + col;
        buffer_ptr[1 + i * VERTEX_SIZE + offset] = y - row;
        buffer_ptr[2 + i * VERTEX_SIZE + offset] = ((texID % NUM_CELLS) * SPRITE_DIMENSIONS + SPRITE_DIMENSIONS * col + gapX) / (float)ATLAS_DIMENSIONS;
        buffer_ptr[3 + i * VERTEX_SIZE + offset] = 1.0f - ( ((texID / NUM_CELLS) * SPRITE_DIMENSIONS + SPRITE_DIMENSIONS * row + gapY) / (float)ATLAS_DIMENSIONS );
    }
    VBO.unMapBuffer();
    VBO.unBind();
}
...
void TileMapRenderer::addIndexData(){
    ...
    int numQuads = (Camera::getCamViewWidth() + 1) * (Camera::getCamViewHeight() + 2 + 1);
